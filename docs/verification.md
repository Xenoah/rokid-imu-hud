...
| 模擬IMU回帰試験 | 24項目合格 | `CoreTests`＋`CalibrationTests`。ログは `artifacts/core-test-results.txt` |
| デスクトップ描画 | 8画像生成 | アプリと同一 `HudRenderer`、Java2Dアダプター |
...
| 校正待機の画面 | 確認済み | 共通描画コードから理由・ACC・GYRO・Hzを含む画像生成 |
| 診断CSV書式 | 合格 | 実装の書式をJVMで実行し、32列がヘッダーと一致 |
