...
| 模擬IMU回帰試験 | 39項目合格 | `CoreTests`＋`CalibrationTests`＋`WearCalibrationTests`＋`RecoveryCalibrationTests`。ログは `artifacts/core-test-results.txt` |
...
| 校正待機の画面 | 確認済み | 共通描画コードから理由・ACC・GYRO・Hz・POSE・TILT・GYRO MEANを含む画像生成。追加行の重なりなし |
| 診断CSV書式 | 合格 | 実装の書式をJVMで実行し、35列がヘッダーと一致 |
| 診断スクリプトの失敗経路 | 3項目合格 | 疑似ADB。インストール失敗、Activity起動エラー、即時終了時のログ保存 |
| Gradle / Android lint | **未実施** | Gradle配布先へのネットワーク制限が継続 |
| 修正版0.1.4の実機・エミュレーターでの起動 | **未実施** | 利用可能な実行環境なし |
...
35. 実際の4.5°/sのPitch回転は生加速度方向で検出し、OS／自前融合の両方で校正を拒否。
...
項目28・31は0.1.3から判定の意味を変更しています。今回の修正は「頭を静止させている」という前提で大きめの一定オフセットを校正するため、以前拒否していた一定Yawも通る場合があります。39件合格を、以前の運動拒否性能がすべて同じまま維持されたと解釈しないでください。根拠と新旧比較は [calibration-fix-0.1.4.md](calibration-fix-0.1.4.md)。
