...
| 模擬IMU回帰試験 | 32項目合格 | `CoreTests`＋`CalibrationTests`＋`WearCalibrationTests`。ログは `artifacts/core-test-results.txt` |
...
| 0.1.3装着中校正の回帰 | 8項目合格 | 旧版でうち4件の失敗を再現。模擬データのみ |
...
28. 3.69°/sの持続Yawを静止バイアスと誤認しない。
29. 始点へ戻る大きな往復運動を校正窓の途中で棄却。
30. Quaternion符号の反転q→−qで誤った回転を検出しない。
31. OS姿勢がない経路では持続Yawに対する厳しい判定を維持。
